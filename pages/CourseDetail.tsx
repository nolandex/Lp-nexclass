...
const CourseDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchCourse = async () => {
      ...
      if (data) {
        ...
        setIsAdded(myCourses.some((c: any) => c.id === data.id));
  ...
  const handleAddRemoveCourse = () => {
    ...
    try {
      const myCourses = JSON.parse(localStorage.getItem("myCourses") || "[]");
      if (isAdded) {
        const updatedCourses = myCourses.filter((c: any) => c.id !== course.id);
  ...
  if (!course) {
    return (
      <div className="min-h-screen bg-card flex items-center justify-center pb-16">
        ...
        <Tabs defaultValue="modules" className="w-full">
            ...
            {course.modules && course.modules
              ...
              >
                ...
                <div className="flex-1 flex items-baseline gap-3">
                  <span className="text-xs font-mono text-muted-foreground w-5 text-right">{String(index + 1).padStart(2, '0')}</span>
          ...
          <TabsContent value="description">
            <div className="bg-card rounded-xl p-6 shadow-sm">
              <h3 className="text-lg font-semibold text-foreground mb-3">Deskripsi Kursus</h3>
